{ config, lib, pkgs, ... }: {
  sops.secrets = let
    ...
    mkPasswords = lib.mkMerge (lib.lists.forEach (builtins.filter (user:
      let inherit (config.users.users."${user}") uid;
      in uid != null && uid >= 1000 && uid <= 1010)
      (builtins.attrNames config.users.users)) (user: {
        "user_passwords/${user}" = {
          neededForUsers = true;
  ...
  users = {
    ...
    users = {
      elxreno = {
        ...
        hashedPasswordFile =
          lib.mkDefault config.sops.secrets."user_passwords/elxreno".path;
      ...
      alena = lib.mkIf (config.device == "AMD-Desktop") {
        ...
        hashedPasswordFile =
          lib.mkDefault config.sops.secrets."user_passwords/alena".path;
        openssh.authorizedKeys.keys =
          config.users.users.elxreno.openssh.authorizedKeys.keys;
        isNormalUser = true;
        uid = 1001;
      };
    };
